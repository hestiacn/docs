...
deprecation_notice() {
	...
	distro_version=$2
	echo
    printf "\033[91;1m弃用警告\033[0m\n"
    printf "    此Linux发行版（\033[1m%s %s\033[0m）已达到生命周期终点，并且此脚本将不再为其提供支持。\n" "$发行版" "$发行版版本"
    echo   "    此发行版将不再发布更新或安全修复，建议用户"
    echo   "    升级到当前受支持的$发行版版本。"
    echo
    printf   "现在按 \033[1mCtrl+C\033[0m 以中止此脚本，或等待安装继续。"
	echo
	sleep 10
}
...
echo_docker_as_nonroot() {
	...
	if version_gte "20.10"; then
		echo "要以非特权用户身份运行 Docker，请考虑设置"
		echo "用户的无根模式下的 Docker 守护程序："
		echo
		echo " dockerd-rootless-setuptool.sh 安装 "
		echo
		echo "访问 https://docs.docker.com/go/rootless/ 以了解无根模式。"
		echo
	fi
	echo
	echo "将 Docker 守护程序作为完全特权服务运行，但授予非 root 权限"
	echo "用户访问，请参阅 https://docs.docker.com/go/daemon-access/"
	echo
	echo "警告：对特权 Docker 守护程序上的远程 API 的访问是等效的"
	echo " 添加到主机上的 root 访问权限。请参阅'Docker 守护程序攻击文档'"
	echo " 文档了解详细信息： https://docs.docker.com/go/attack-surface/"
	echo
	echo "================================================================================"
	echo
}
...
check_forked() {
	...
	if command_exists lsb_release; then
		...
		if [ "$lsb_release_exit_code" = "0" ]; then
			...
			cat <<-EOF
			您使用的 '$lsb_dist' 版本是 '$dist_version'.
			EOF
			...
			cat <<-EOF
			上游版本 '$lsb_dist' 版本是 '$dist_version'.
			EOF
		else
...
do_install() {
	echo "执行 docker 安装脚本，提交: $SCRIPT_COMMIT_SHA"

	if command_exists docker; then
		cat >&2 <<-'EOF'
			警告：“docker”命令似乎已存在于此系统上。

            如果您已经安装了 Docker，则此脚本可能会导致问题，即
			为什么我们显示此警告并提供取消
			安装。

            如果您使用此脚本安装了当前的 Docker 软件包，并且正在使用它
			再次更新 Docker，您可以放心地忽略此消息。
	...
	if is_wsl; then
		echo
		echo "检测到 WSL：建议使用适用于 Windows 的 Docker Desktop."
		echo "请从以下位置获取 Docker Desktop https://www.docker.com/products/docker-desktop/"
		echo
		cat >&2 <<-'EOF'
	...
	case "$lsb_dist" in
		ubuntu|debian|raspbian)
			...
			(
				...
				echo "正在创建/etc/apt/keyrings目录"
				$sh_c 'install -m 0755 -d /etc/apt/keyrings'
				echo "正在下载Docker的GPG密钥"
				$sh_c "curl -fsSL \"$DOWNLOAD_URL/linux/$lsb_dist/gpg\" -o /etc/apt/keyrings/docker.asc"
				echo "已设置Docker的GPG密钥权限"
				$sh_c "chmod a+r /etc/apt/keyrings/docker.asc"
				echo "正在添加Docker的APT源"
			...
			if [ -n "$VERSION" ]; then
				if is_dry_run; then
					...
					search_command_description="搜索Docker CE版本，提取匹配'$pkg_pattern'的最高版本号的包版本"
					pkg_version="$($sh_c "$search_command")"
					echo "信息: 在 docker版本库 中搜索'$VERSION'匹配的存储库版本"
					echo "信息: $search_command_description"
		...
		sles)
			...
			(
				...
				if ! is_dry_run; then
						cat >&2 <<-'EOF'
						警告！！
						openSUSE 仓库 （https://download.opensuse.org/repositories/security:/SELinux） 将启用。
						是否要继续？
						您现在可以按 Ctrl+C 中止此脚本.
						EOF
						( set -x; sleep 30 )
