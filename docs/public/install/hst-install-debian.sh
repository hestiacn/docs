...
software="acl apache2 apache2-suexec-custom apache2-utils at bc bind9 bsdmainutils bsdutils
...
echo
echo "如果您的服务器位于中国大陆境内预计时间在10-50分钟左右安装完成！感谢您的耐心等待！"
echo
...
TARGET_FILE="$HESTIA/web/list/notifications/index.php"
sed -i 's/format("d M Y, H:i")/format("Y年m月d日 H时i分s秒")/g' "$TARGET_FILE"
sed -i 's/format("d F Y, H:i:s")/format("Y年m月d日 H时i分s秒")/g' "$TARGET_FILE"
...
echo -e "祝贺您!\n\n您已成功在 $os_name 服务器上安装 Hestia  $HESTIA_INSTALL_VER 服务器控制面板.

开始之前请在你的域名解析处添加上服务器解析记录:(例如. demo.hestiacp.com)

请在浏览器输入您配置好的服务器主机地址，(例如. demo.hestiacp.com) 

里面有详细的常见错误处理方案！让您在使用 Hestia 时不至于手足无措！
