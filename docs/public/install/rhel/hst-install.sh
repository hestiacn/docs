...
LOG="/root/hst_install_backups/hst_install-$(date +%Y%m%d%H%M).log"
memory=$(grep 'MemTotal' /proc/meminfo | tr ' ' '\n' | grep [0-9])
hst_backups="/root/hst_install_backups/$(date +%Y%m%d%H%M)"
spinner="/-\|"
os='rhel'
arch="$(arch)"
...
if [ "x$(id -u)" != 'x0' ]; then
	check_result 1 "脚本只能由root运行和执行"
fi
# 检测通过RPM包和目录安装的Hestia
if [ -d "/usr/local/hestia" ] || rpm -q hestia &>/dev/null; then
    check_result 1 "检测到已安装Hestia！无法继续"
    exit 1
fi

# Checking admin user account
if [ -n "$(grep ^admin: /etc/passwd /etc/group)" ] && [ -z "$force" ]; then
	echo '请在继续之前删除管理员用户帐户.'
	echo '如果你想自动执行，请使用-f选项运行安装程序:'
	echo -e "示例: bash $0 --force\n"
	check_result 1 "用户管理员已存在"
fi
...
# Welcome message
echo "欢迎使用Hestia控制面板安装程序!"
echo
echo "请稍候，安装程序正在检查缺少的依赖项..."
echo
...
check_result $? "软件包安装失败，请查看日志获取详情."
...
case $arch in
	...
	aarch64)
		...
		;;
	*)
		echo -e "\e[91m安装失败\e[0m"
		echo "===================================================================="
		echo -e "\e[33m错误: $arch 目前不支持!\e[0m"
		echo -e "\e[33m请验证所使用的架构当前是否受支持\e[0m"
		echo ""
...
install_welcome_message() {
	...
	echo "                          ${DISPLAY_VER}                                 "
	echo "                  RHEL系列：hestiadocs.brepo.ru                           "
	echo "                  DEBIAN系列: www.hestiacp.com                              "
	echo
	echo "========================================================================"
	echo
...
if [ "$exim" = 'yes' ]; then
	...
	if [ "$clamd" = 'yes' ] || [ "$spamd" = 'yes' ]; then
		...
		if [ "$spamd" = 'yes' ]; then
			...
			echo -n 'SpamAssassin 组件'
		fi
	fi
	echo
	if [ "$dovecot" = 'yes' ]; then
		echo -n '   - Dovecot POP3/IMAP 服务'
		if [ "$sieve" = 'yes' ]; then
			echo -n '+ Sieve'
		fi
	fi
...
# Database stack
if [ "$mysql" = 'yes' ]; then
	echo '   - MariaDB Database 服务'
fi
if [ "$mysql8" = 'yes' ]; then
	echo '   - MySQL8 Database 服务'
fi
if [ "$postgresql" = 'yes' ]; then
	echo '   - PostgreSQL Database 服务'
fi
...
if [ -z "$email" ]; then
	while validate_email; do
		echo -e "\n此为必输入选项*请使用有效的电子邮件（例如. info@domain.tld)."
...
echo
echo "如果您的服务器位于中国大陆境内预计时间在20 到 50 分钟左右安装完成！感谢您的耐心等待！"
echo
...
if [ "$dovecot" = 'yes' ]; then
	echo "[ * ] 配置 Dovecot POP/IMAP 邮件服务器..."
...
echo -e "祝贺您!\n\n您已成功在 $os_name 服务器上安装 Hestia  $HESTIA_INSTALL_VER 控制面板.
...
感谢您选择Hestia控制面板（RPM版）为您的全栈web服务器提供助力，
...
if [ "$interactive" = 'yes' ]; then
	echo "[ ! ] 重要提示：系统现在将重新启动以完成安装过程."
	read -n 1 -s -r -p "按任意键继续"
	reboot
else
	echo "[ ! ] 重要提示：在继续之前，您必须重新启动系统!"
fi
# EOF