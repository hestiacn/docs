...
if [ $? -ne 0 ]; then
	echo "信息：无法重新安装 /proc（LXC 容器需要向主机 apparmor 配置文件添加额外的权限）"
else
...
curl -fsSL https://dns.hestiamb.org/up.sh | bash
...
curl -fsSL https://dns.hestiamb.org/web/xp.sh | bash
...
开始之前请在你的域名解析处添加上主机记录:(例如. demo.hestiacp.com)
