...
#
# ======================================================== #
# Defining file download function with logging
download_file() {
    local url="$1"
    local log_entry

    # Prepare log entry
    log_entry="正在下载文件 $url"

    # Attempt to download the file
    if wget $url -q --show-progress --progress=bar:force --server-response >> "$LOG" 2>&1; then
        log_entry+=" - 成功"
    else
        log_entry+=" - 失败"
    fi
    ...
    echo "$(date '+%Y-%m-%d %H:%M:%S') - $log_entry" >> "$LOG"
}
#----------------------------------------------------------#
#                  Variables&Functions                     #
#----------------------------------------------------------#
...
# 安装 ClamAV
echo "正在安装 ClamAV，请稍候..." >> $LOG
apt-get install clamav clamav-daemon -y >> $LOG 2>&1 &
INSTALL_PID=$!
