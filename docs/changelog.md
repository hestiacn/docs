...
## [1.10.5] - 2026-09-14 - 服务版本发布

### 安全性

- 验证用户角色/会话是否被篡改，并修复被暂停用户有时未被禁用登录的问题 #5706
- 修复 `v-unsuspend-user` 使用了错误的 `POLICY_USER_VIEW_SUSPENDED` 比较逻辑，导致解除暂停时并不总能解锁账户 #5711
- 改进只读/演示模式的强制限制，封堵通过直接 URL 访问（如 `/add/cron/`）、开放重定向以及管理员“以用户身份登录”流程的绕过漏洞 #5688
- 改进配置文件解析和保留键处理，防止通过构造的配置值进行注入攻击 #5698

### 变更

- 移除对 Debian 11（Bullseye）的支持 #5654
- 升级快速安装应用版本：Dolibarr、Drupal、Grav、Joomla、OpenCart、PrestaShop #5700
- 升级 hestia-php 至 8.5.10 #5678
- 升级 OpenSSL 至 3.5.8 #5658
- 更新 PCRE 至 10.48 #5673
- 更新 Roundcube 至 1.7.4 #5690
- 升级文件管理器至 7.16.2 #5695
- 增强备份详情页面，为项目类型添加图标 #5679
- 更新依赖项 #5666 #5693 #5694
- 为暂停用户功能添加缺失的测试 #5710

### 修复

- 修复反转的数据库限制检查条件，该问题可能导致用户在配额内无法创建数据库 #5640
- 修复远程数据库的 phpMyAdmin 单点登录问题，以及重定向到错误服务器的问题 #5454
- 重命名 phpMyAdmin 临时目录配置，使其最后加载 #5655
- 修复 restic 完整恢复功能，确保恢复所有对象，并修正 cron 二进制文件名 #5709
- 修复备份主机设置时传递 `$raw_password` 而非 `$password` 给 Backblaze B2 凭据的问题 #5697
- 修复在 NAT/反向代理环境后运行时的相关问题 #5689
- 为 Fail2Ban 操作和防火墙状态实现锁定机制，并修复原始 IP CIDR 处理 #5699
- 修复 Debian 13 和 Ubuntu 26.04 上的 ProFTPd #5677
- 为超过 64 个字符的域名生成 SSL 证书时省略 CN #5676
- 修复全局 SMTP 中继密码验证 #5665
- 验证空数据库别名 #5663
- 修复域名被暂停时 webstats 失败导致的 Apache2 日志轮转问题 #5685
- 修复现有 Exim 安装的“Rejected because”DNSBL 消息 #5683
- 修复移动端视图下操作按钮图标与文字重叠的问题 #5680

## [1.10.4] - 2026-08-24 - 服务版本发布

### 安全修复

- 修复 sudo 命令通过 auth.log / journalctl 意外泄露密码的问题 #5648
- 为 `v-schedule-user-restore` 添加所有者验证 #5645

### 变更

- 默认禁用 Roundcube 的 IMAP 和 SMTP 调试模式，以防止磁盘空间问题 #5642
- 为所有 `v-list-*-ssl` 脚本添加 Hestia 演示模式检查 #5639
- 将 Ubuntu Docker 标签更新至 v26 #5438
- 将依赖项 `@xterm/xterm` 更新至 v6 #5627

### 修复

- 修复因 MySQL HeatWave 中等密码策略导致数据库临时用户创建失败的问题 #5453
- 修复 SFTP 备份轮换中的错误，该错误导致备份存储无法正确轮换 #5646
- 修复返回链接和访问密钥按钮指向错误用户的问题 #5635 #5638
- 修复迁移脚本未将 `imap_sieve` 插件添加到 IMAP 配置文件的问题 #5637
- 修复序列号中的拼写错误，该错误导致默认 DNS 序列号不一致 #5632
- 改进文件管理器的安装验证和错误日志记录 #5634
...
- 修复：Webmail DNS 记录与别名不一致的问题 (#5074)
- 修复：重建邮件域时删除 Webmail SSL 配置的问题 (#5354)
- 更新 Roundcube 到版本 1.6.16 (#5357)

---

## [1.9.5] - 2026-05-28 - 服务更新版

### 新功能

- 在快速安装应用中增加对 PHP 8.4 的支持
- 改进安全性和性能优化

### Bug 修复

- 修复：清理参数处理并改进键值对解析 (#5309)
- 修复：Apache 2.4.64 更新后所有 Web 和邮件域出现 421 错误的问题
- 修复：将默认 SOA 重试值设置为 1800 以符合 DENIC 规范
- 修复更改 Web 域名时域别名替换逻辑的问题
- 修复 ipv4_cidr 验证
- 更新 magento.tpl / magento.stpl 以支持健康检查
- 在仪表板中显示用户和带宽配额
- 在保留检查前对备份文件列表进行排序
- 改进 Spamhaus DQS 查询的日志记录，不暴露查询密钥
- 更新 Roundcube 到版本 1.6.11
- 从 Debian 安装程序中移除 apache2-suexec-pristine 软件包
- 修复域名重定向无法挂起的问题
- 确保 hestiaweb 用户 crontab 末尾有换行符
- 允许在添加用户名到 SMTP 中继时使用斜杠
- 防止空用户变量影响多个脚本
- 修复编辑 hestiaweb 的面板定时任务
- 修复 Ubuntu 上缺失的 proftpd-mod-crypto 依赖
- 修复 Hestia 验证证书链的方式
- 更新文件管理器到最新版本的类

## [1.9.4] - 2025-07-21 - 服务版本
...
## [1.9.1] - 2025-01-04 - 版本更新发布-紧急修复错误

- 修复了webmail/phpMyAdmin无法使用的问题

## [1.9.0] - 2025-01-04 - 功能/主要版本更新发布
...
## [1.8.7] - 2023-08-24 - 版本更新发布

Bug修复

- 修复 v-update-whitelabel-logo 中的拼写错误

## [1.8.6] - 2023-07-25 - 版本更新发布
特征
...
## [1.8.4] - 2023-08-01 - 版本更新发布-根据发布包时间填写（参考）
...
## [1.5.13] - 2022-04-28 - 版本更新发布

Bug修复

- 修复了通过 GUI 添加/更改 DNS 记录的问题。 (#2557)

## [1.5.12] - 2022-04-28 - 版本更新发布
...
## [1.4.1] - 2021-05-27 - 错误修复

- 修复了启用 2FA 登录的错误

## [1.4.0] - 2021-05-27 - 主要版本（功能/质量更新）
